...
import { tBodyPart, tMuscle, tEquipment, translateInstruction } from '../utils/translations'
...
export default function ExerciseDetail() {
  ...
  const { name, gifUrl, bodyParts, equipments, targetMuscles, secondaryMuscles, instructions } = exercise
      ...
      <div className="lg:grid lg:grid-cols-[400px_1fr] gap-10">
        ...
        <div className="flex flex-col gap-4">
          ...
          <div className="space-y-3">
            <TagRow label="Parte del cuerpo"     items={bodyParts}        translate={tBodyPart} />
            <TagRow label="Músculo objetivo"     items={targetMuscles}    translate={tMuscle} variant="green" />
            {secondaryMuscles?.length > 0 && (
              <TagRow label="Músculos secundarios" items={secondaryMuscles} translate={tMuscle} />
            )}
            <TagRow label="Equipamiento"         items={equipments}       translate={tEquipment} />
          </div>
...
function TagRow({ label, items, translate, variant }) {
  return (
    <div>
      <p className="text-xs text-gray-500 font-medium uppercase tracking-wider mb-1.5">{label}</p>
      <div className="flex flex-wrap gap-1.5">
        {items.map((item) => (
          <span key={item} className={variant === 'green' ? 'badge badge-green' : 'badge'}>
            {translate(item)}
          </span>
        ))}
      </div>
